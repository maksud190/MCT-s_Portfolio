...
export default function NotificationBell() {
  ...
  const handleNotificationClick = (notification) => {
    ...
    if (notification.type === "project_approved" || 
        ...
        notification.type === "project_featured") {
      if (notification.project) {
        // Go to the project if approved/featured
        if (notification.type === "project_approved" || notification.type === "project_featured") {
          navigate(`/project/${notification.project._id || notification.project}`);
        } else {
          // Go to profile if rejected (to see rejection reason)
          navigate("/profile");
        }
      } else {
        navigate("/profile");
  ...
  // 🔥 Updated notification icons with approval types
  const getNotificationIcon = (type) => {
    const icons = {
      like: (
        <svg className="w-5 h-5 text-red-500" fill="currentColor" viewBox="0 0 20 20">
      ...
      // 🔥 NEW: Approval notification icons
      project_approved: (
  ...
  const getNotificationBgColor = (notif) => {
    if (!notif.read) {
      if (notif.type === "project_approved") {
        return "bg-green-50/50";
      } else if (notif.type === "project_rejected") {
        return "bg-red-50/50";
      } else if (notif.type === "project_featured") {
                ...
                {notifications.length > 0 ? (
                  notifications.map((notif) => (
                    <div
                      key={notif._id}
                      onClick={() => handleNotificationClick(notif)}
                      className={`p-4 border-b border-gray-100 hover:bg-blue-50 cursor-pointer transition-all group ${getNotificationBgColor(notif)}`}
                    >
                      <div className="flex gap-3">
                        {/* 🔥 Show admin icon for approval notifications */}
                        <div className="flex-shrink-0">
                          {(notif.type === "project_approved" || 
                            notif.type === "project_rejected" || 
                            notif.type === "project_featured") ? (
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-start justify-between gap-2 mb-1">
                            <p className="text-sm text-gray-700 break-words">
                              {/* 🔥 Show "Admin" for approval notifications */}
                              {(notif.type === "project_approved" || 
                                notif.type === "project_rejected" || 
                                notif.type === "project_featured") ? (
                            ...
                            {!notif.read && (
                              <div className="flex items-center gap-1">
                                <div className={`w-2 h-2 rounded-full ${
                                  notif.type === "project_approved" ? "bg-green-600" :
                                  notif.type === "project_rejected" ? "bg-red-600" :
                                  notif.type === "project_featured" ? "bg-yellow-600" :
                                  "bg-blue-600"
                                }`}></div>
                                <span className={`text-xs font-semibold ${
                                  notif.type === "project_approved" ? "text-green-600" :
                                  notif.type === "project_rejected" ? "text-red-600" :
                                  notif.type === "project_featured" ? "text-yellow-600" :
                                  "text-blue-600"
                                }`}>New</span>
                              </div>
                            )}
