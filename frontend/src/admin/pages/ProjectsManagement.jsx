...
export default function ProjectsManagement() {
  ...
  const handleApproveProject = async (projectId, currentStatus) => {
    ...
    if (!window.confirm(`${action.charAt(0).toUpperCase() + action.slice(1)} this project?`)) return;
  ...
  const handleToggleFeatured = async (projectId) => {
    try {
      const token = localStorage.getItem('token');
      await API.put(`/admin/projects/${projectId}/featured`, {}, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleDeleteProject = async (projectId, title) => {
    if (!window.confirm(`⚠️ Delete project "${title}"?\n\nThis will also delete all comments. This action cannot be undone.`)) return;
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-6">
        {loading ? (
          ...
          projects.map((project) => (
            <div key={project._id} className="bg-white dark:bg-gray-800 rounded-lg shadow overflow-hidden hover:shadow-lg transition-shadow">
              {/* Thumbnail */}
