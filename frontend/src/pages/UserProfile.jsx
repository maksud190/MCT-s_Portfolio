      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 mb-8">
        <div className="flex flex-col md:flex-row items-center md:items-start gap-6">
          ...
          <div className="flex-1 text-center md:text-left">
            ...
            {(userData.studentId || userData.batch || userData.batchAdvisor || userData.batchMentor) && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm bg-gray-50 dark:bg-gray-700/50 rounded-lg p-4">
                {userData.studentId && (
                  <div>
                    ...
                    <span className="ml-2 text-gray-800 dark:text-white">{userData.studentId}</span>
                  </div>
                )}
                {userData.batch && (
                  <div>
                    <span className="font-semibold text-gray-600 dark:text-gray-400">Batch:</span>
                    <span className="ml-2 text-gray-800 dark:text-white">{userData.batch}</span>
                  </div>
                )}
                {userData.batchAdvisor && (
                  <div>
                    <span className="font-semibold text-gray-600 dark:text-gray-400">Advisor:</span>
                    <span className="ml-2 text-gray-800 dark:text-white">{userData.batchAdvisor}</span>
                  </div>
                )}
                {userData.batchMentor && (
                  <div>
                    <span className="font-semibold text-gray-600 dark:text-gray-400">Mentor:</span>
                    <span className="ml-2 text-gray-800 dark:text-white">{userData.batchMentor}</span>
                  </div>
                )}
              </div>
            )}
